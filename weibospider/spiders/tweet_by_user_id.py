...
class TweetSpiderByUserID(Spider):
    ...
    def start_requests(self):
        ...
        config_path = os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "config", "spider_config.toml"
        )
        config = toml.load(config_path)
        spider_config = config["tweet_by_user_id"]

        # 从配置中获取参数
        user_ids = spider_config["user_ids"]
        is_crawl_specific_time_span = spider_config["is_crawl_specific_time_span"]

        if is_crawl_specific_time_span:
            time_config = spider_config["time_span"]
            ...
            end_time = datetime.datetime(
                year=time_config["end_year"],
                month=time_config["end_month"],
                day=time_config["end_day"],
            )
        ...
        for user_id in user_ids:
            ...
            if not is_crawl_specific_time_span:
                yield Request(
                    url, callback=self.parse, meta={"user_id": user_id, "page_num": 1}
                )
            else:
                # 切分成10天进行
                ...
                while tmp_start_time <= end_time:
                    ...
                    tmp_url = (
                        url
                        + f"&starttime={int(tmp_start_time.timestamp())}&endtime={int(tmp_end_time.timestamp())}"
                    )
                    yield Request(
                        tmp_url,
                        callback=self.parse,
